...
anywhere. Laid out like a gateway dashboard — a dark header, a full-bleed **Perspective workspace**, a
right control gutter, a footer — it's the **REST + Perspective** path a real gateway uses, so deliberately
**no transports**:
...
- **A live Perspective blotter** — `PerspectivePanel` (themed via its `theme` setter) showing the
  `orders` table; the bulk data rides Perspective's *own* websocket (Mode B), so a sent order appears at
  once. The header's **view selector** re-pushes the blotter layout (flat blotter ↔ by-symbol roll-up).
