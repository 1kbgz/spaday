...
def classes(manifest_path: str, name: str | None = None) -> dict[str, type[Component]] | type[Component]:
...
def render(component_schemas: list[dict], *, source: str = "a manifest") -> str:
    ...
    typing_imports = [n for n in ("Any", "Literal") if n in used]
...
def _render_class(schema: dict) -> _ClassBody:
    ...
    imports: set = {"Any"}  # **props: Any is always present
    params = ["self", "*children: Child", "key: str | None = None"]
    assigns = []
    seen = set()
    for prop in schema["props"]:
        attr = prop["name"]
...
def _annotation(ty: Any) -> tuple:
    ...
    if inner == "Any":
        return "Any", used  # Any already includes None
    return f"{inner} | None", used
