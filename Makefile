...
jupyterlite:  ## build the JupyterLite demo site into dist/lite (needs the Pyodide wheel)
	...
	python -m pip download --no-deps --only-binary=:all: --platform pyemscripten_2026_0_wasm32 --python-version 314 --implementation cp --abi cp314 --dest examples/lite/pypi "transports==0.9.0"
